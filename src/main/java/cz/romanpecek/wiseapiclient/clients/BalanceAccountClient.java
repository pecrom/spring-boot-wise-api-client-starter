...
public interface BalanceAccountClient {
    ...
    BalanceAccount create(@PathVariable Long profileId, @RequestHeader(name = "X-idempotence-uuid") UUID idempotenceUUID, @RequestBody NewBalanceAccount newBalanceAccount);
    ...
    ResponseEntity<byte[]> getStatementByProfileIdAndIdAndType(@PathVariable Long profileId, @PathVariable Long balanceId, @PathVariable String responseType, @RequestParam @DateTimeFormat(pattern = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'") OffsetDateTime intervalStart, @RequestParam @DateTimeFormat(pattern = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'") OffsetDateTime intervalEnd, @RequestParam StatementType type);

    @PostMapping(path = "/v2/profiles/{profileId}/balance-movements")
    ConversionResult convertBetweenBalances(@PathVariable Long profileId, @RequestHeader(name = "X-idempotence-uuid") UUID idempotenceUUID, @RequestBody Long quoteId);

    @PostMapping(path = "/v2/profiles/{profileId}/balance-movements")
    ConversionResult moveBetweenBalances(@PathVariable Long profileId, @RequestHeader(name = "X-idempotence-uuid") UUID idempotenceUUID, @RequestBody MoveBetweenBalances moveBetweenBalances);
}
